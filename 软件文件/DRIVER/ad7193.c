...
unsigned char ad7193_init(void)
{
    ...
    regVal = ad7193_get_register_value(AD7193_REG_ID, 1, 1);
	
    if((regVal & AD7193_ID_MASK) != ID_AD7193)  status = 0;
 ...
 * @return none.
*******************************************************************************/
void ad7193_set_register_value(unsigned char registerAddress,
                             ...
                             unsigned char modifyCS)
{
    ...
    writeCommand[0] = AD7193_COMM_WRITE|AD7193_COMM_ADDR(registerAddress);
    while(bytesNr > 0)
    {
...
*******************************************************************************/
unsigned long ad7193_get_register_value(unsigned char registerAddress,
                                      unsigned char bytesNumber,
                                      unsigned char modifyCS)
{
    ...
    registerWord[0] = AD7193_COMM_READ |AD7193_COMM_ADDR(registerAddress);
...
void ad7193_channel_select(unsigned short channel)
{
    ...
    oldRegValue = ad7193_get_register_value(AD7193_REG_CONF, 3, 1);
    oldRegValue &= ~(AD7193_CONF_CHAN(0x3FF));
    newRegValue = oldRegValue | AD7193_CONF_CHAN(1 << channel);   
    ad7193_set_register_value(AD7193_REG_CONF, newRegValue, 3, 1);
}
...
void ad7193_calibrate(unsigned char mode, unsigned char channel)
{
    ...
    ad7193_channel_select(channel);
    oldRegValue = ad7193_get_register_value(AD7193_REG_MODE, 3, 1);
    oldRegValue &= ~AD7193_MODE_SEL(0x7);
    newRegValue = oldRegValue | AD7193_MODE_SEL(mode);
    PMOD1_CS_LOW; 
    ad7193_set_register_value(AD7193_REG_MODE, newRegValue, 3, 0); // CS is not modified.
...
void ad7193_range_setup(unsigned char polarity, unsigned char range)
{
    ...
    newRegValue = oldRegValue|(polarity * AD7193_CONF_UNIPOLAR)|AD7193_CONF_GAIN(range); 
    ad7193_set_register_value(AD7193_REG_CONF, newRegValue, 3, 1);
...
unsigned long ad7193_continuous_readavg(unsigned char sampleNumber)
{
    ...
    command = AD7193_MODE_SEL(AD7193_MODE_CONT)|AD7193_MODE_CLKSRC(AD7193_CLK_INT)|AD7193_MODE_RATE(0x060);
    PMOD1_CS_LOW;
    ad7193_set_register_value(AD7193_REG_MODE, command, 3, 0); // CS is not modified.
...
unsigned long ad7193_temperature_read(void)
{
    ...
    ad7193_range_setup(0, AD7193_CONF_GAIN_1); // Bipolar operation, 0 Gain.
