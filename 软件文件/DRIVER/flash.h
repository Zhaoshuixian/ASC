...
/* Base address of the Flash sectors  128PGS*/
...
/*FLASH地址信息*/
#define STM32_FLASH_BASE_ADDR (ADDR_FLASH_PAGE_0)
#define STM32_FLASH_END_ADDR  (ADDR_FLASH_PAGE_127+FLASH_PAGE_SIZE - 1)

#define FLASH_OK          (0)
#define FLASH_ERR         (1)
#define FLASH_CHECK_ERR   (2)
 
typedef unsigned char flash_status_t;

flash_status_t stm32_flash_init(void);
flash_status_t stm32_flash_deinit(void);
flash_status_t stm32_flash_erase(unsigned int er_addr);
flash_status_t stm32_flash_write(unsigned char *wr_buff,unsigned int wr_addr,unsigned int wr_len);
