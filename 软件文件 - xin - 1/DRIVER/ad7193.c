...
void ad7193_setpower(unsigned char pwrMode)
{
    ...
    //  unsigned char rdy_status =0x80;
    //  ad7193_set_register_value(AD7193_REG_MODE, newPwrMode, 3, 1); 
    // do
    // {
    //     rdy_status=ad7193_get_register_value(AD7193_REG_STAT,1,0);
    // }
    //while(rdy_status&0x80); 	
}
 ...
 */      
    ...
    for(count = 0; count < sampleNumber; count++)
    {
        ...
        do
        {
            rdy_status=ad7193_get_register_value(AD7193_REG_STAT,1,0);//读状态寄存器的RDY标志位
        }
        while(rdy_status&0x80);
